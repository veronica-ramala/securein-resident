...
export default function LocalConnectScreen() {
  ...
  const [favorites, setFavorites] = useState(new Set(['1', '3', '8'])); // Sample favorites
  const [recentCalls, setRecentCalls] = useState(['2', '5', '1']); // Sample recent calls
  ...
  const getProfessionImage = (professionId) => {
    if (!imageConfig.enableImages) return null;
    return imageConfig.professionImages[professionId.toLowerCase()] || null;
  };
  ...
  const getProfessionCount = (profession) => {
  ...
  const toggleFavorite = (neighborId) => {
  ...
  const handleCategorySelect = (profession) => {
            ...
            <View style={styles.avatarWrapper}>
              <View style={[styles.smallAvatar, !isAvailable && styles.unavailableAvatar]}>
                {getProfileImage(neighbor.id) ? (
                  <Image
                    source={{ uri: getProfileImage(neighbor.id) }}
                  ...
                  >
                    <View style={[styles.professionIconContainer, { backgroundColor: profession.color + '20' }]}>
                      {getProfessionImage(profession.id) ? (
                        <Image
                          source={{ uri: getProfessionImage(profession.id) }}
